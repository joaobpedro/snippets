...
if __name__ == '__main__':
    ...
    output_df = pd.DataFrame(columns=['File Name','ID', 'Hs','Tp','Case Type', 'Comb Tens Curv', 'Comb Curv Tens', 'top max utilization', 'max bs strain', 'Comb BM SF', 'Comb SF BM', 'min bo tension', 'max bo util', 'max bo curv', 'min tdp tension', 'max tdp utilization','Comb BO Tens Curv', 'Comb BO Curv Tens'])
    ...
    output_df['Comb BO Tens Curv'] = Results_ALL['max_bo_tension'] +'_'+Results_ALL['asso_curv'].astype(str)
    output_df['Comb BO Curv Tens'] = Results_ALL['max_bo_curv'] +'_'+Results_ALL['asso_tension'].astype(str)
    ...
    pivot_table = pd.pivot_table(output_df,
                ...
                index = ['ID','Case Type'],
                aggfunc=aggfunc)
    pivot_table.to_csv('pivot_test.csv')