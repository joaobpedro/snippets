...
def Processor(name):
    ...
    bs_strain = stiff.RangeGraph('Max bending strain', None, None, arSpecifiedArclengths(0.15, 8.7))
    ...
    max_tdp_tension, assoc_tdp_curv, max_tdp_curv, assoc_tdp_tension = max_couples(tdp_tension, tdp_curvature, mode='double')
    
    #identifiers:
    id, hs, tp, case_type = get_hs_tp_type(name)
    results = [name,id, hs, tp, case_type,f"{max_top_tension:015.9f}",ass_curv,f"{max_top_curv:015.9f}",ass_tension,max_top_util,max_strain,f"{max_top_bm:015.9f}",ass_sf,f"{max_top_sf:015.9f}",ass_bm,min_bo_tension,max_bo_util,max_bo_curv,min_tdp_tension,max_tdp_util,f"{max_bo_tension:015.9f}", ass_curv_bo, f"{max_bo_curv_2:015.9f}", ass_tension_bo, f"{max_tdp_tension:015.9f}", assoc_tdp_curv, f"{max_tdp_curv:015.9f}", assoc_tdp_tension]
...
if __name__ == '__main__':
    ...
    Results_ALL = pd.DataFrame(columns=['File Name','ID', 'Hs','Tp','Case Type', 'max top tnesion', 'asso curv', 'max top curv', 'asso tension', 'top max utilization', 'max bs strain', ' max top bm', 'asso sf', 'max top sf', 'asso bm', 'min bo tension', 'max bo util', 'max bo curv', 'min tdp tension', 'max tdp utilization','max_bo_tension', 'asso_curv', 'max_bo_curv', 'asso_tension','max tdp tension', 'assoc_tdp_curv', 'max_tdp_curv', 'asso_tdp_tension'])
    ...
    output_df = pd.DataFrame(columns=['File Name','ID', 'Hs','Tp','Case Type', 'Comb Tens Curv', 'Comb Curv Tens', 'top max utilization', 'max bs strain', 'Comb BM SF', 'Comb SF BM', 'min bo tension', 'max bo util', 'max bo curv', 'min tdp tension','max tdp utilization','Comb BO Tens Curv', 'Comb BO Curv Tens','Comb tdp Tension Curv', 'Comb tdp curv tension'])
    ...
    output_df['Comb tdp Tension Curv'] = Results_ALL['max tdp tension'] +'_'+Results_ALL['assoc_tdp_curv'].astype(str)
    output_df['Comb tdp curv tension'] = Results_ALL['max_tdp_curv'] +'_'+Results_ALL['asso_tdp_tension'].astype(str)
    ...
    values = ['Comb Tens Curv', 'Comb Curv Tens', 'top max utilization', 'max bs strain', 'Comb BM SF', 'Comb SF BM', 'min bo tension', 'max bo util', 'max bo curv', 'min tdp tension', 'max tdp utilization','Comb BO Tens Curv', 'Comb BO Curv Tens', 'Comb tdp Tension Curv', 'Comb tdp curv tension']
    aggfunc = {
        'Comb Tens Curv':max,
        'Comb Curv Tens':max,
        ...
        'Comb tdp Tension Curv':max,
        'Comb tdp curv tension':max
    }
    ...
    to_export = pd.DataFrame(columns=['ID','Case Type', 'max top tnesion', 'asso curv', 'max top curv', 'asso tension', 'top max utilization', 'max bs strain', ' max top bm', 'asso sf', 'max top sf', 'asso bm', 'min bo tension', 'max bo util', 'max bo curv', 'min tdp tension','max tdp utilization','max_bo_tension', 'asso_curv', 'max_bo_curv', 'asso_tension','max tdp tension', 'assoc_tdp_curv', 'max_tdp_curv', 'asso_tdp_tension'])
    ...
    to_export['max tdp tension'] = [i[0] for i in pivot_table['Comb tdp Tension Curv'].str.split('_').values]
    to_export['assoc_tdp_curv'] = [i[1] for i in pivot_table['Comb tdp Tension Curv'].str.split('_').values]

    to_export['max_tdp_curv'] = [i[0] for i in pivot_table['Comb tdp curv tension'].str.split('_').values]
    to_export['asso_tdp_tension'] = [i[1] for i in pivot_table['Comb tdp curv tension'].str.split('_').values]
